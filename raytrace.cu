...
__global__ void getColorAtPixel(int startIdx, int w, int h, float fovtan,
                                float fovtanAspect, v3 origin,
                                unsigned char *rgba, float *sphereList,
                                int numSpheres) {
    // copy the whole sphere list into shared memory
    // this requires 16KiB per block for 1000 spheres, and more than 1000
    // spheres is not yet handled
    ...
    for (; idx < w * h; idx += blockDim.x * gridDim.x) {
        ...
        float distanceX, distanceY, distanceZ, dotProduct, distanceSqr,
            importantPart, d;
        ...
        for (int i = 0; i < numSpheres; i++) {
            ...
            dotProduct =
                dirx * (distanceX) + diry * (distanceY) + dirz * (distanceZ);
            distanceSqr = distanceX * distanceX + distanceY * distanceY +
                          distanceZ * distanceZ;
            importantPart =
                dotProduct * dotProduct - distanceSqr + rad[i] * rad[i];
        ...
        if (bestHitSphere >= 0) {
            ...
            bestNorm.z =
                (pz[bestHitSphere] - bestPt.z) * (1.0 / rad[bestHitSphere]);
...
int main(int argc, char *argv[]) {
    ...
    errorCheck(cudaMemcpy(dspheres, spheres, sizeof(float) * nSpheres * 4,
    ...
    for (int iter = 0; iter < iterations; iter++) {
        getColorAtPixel<<<NUM_BLOCKS, THREADS_PER_BLOCK,
                          nSpheres * 4 * sizeof(float)>>>(
            0, w, h, fvtan, fvtanAsp, orig, drgba, dspheres, nSpheres);
        cudaDeviceSynchronize();
    ...
    cudaMemcpy(rgba, drgba, sizeof(unsigned char) * sz * 4,
