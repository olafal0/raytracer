...
int show(const std::string &caption, const unsigned char *rgba, unsigned w,
         unsigned h) {
    ...
    for (unsigned y = 0; y + jump - 1 < h; y += jump)
        for (unsigned x = 0; x + jump - 1 < w; x += jump) {
            ...
            // make translucency visible by placing checkerboard pattern behind
            // image
            ...
            Uint32 *bufp;
            bufp = (Uint32 *)scr->pixels + (y * scr->pitch / 4) / jump +
                   (x / jump);
    ...
    while (done == 0) {
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT)
                ...
                done = 1; // press any other key for next image
        }
