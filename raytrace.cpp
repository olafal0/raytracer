...
void getColorAtPixel(float *px, float *py, float *pz, float *rad, ray r,
                     int numSpheres, unsigned char color[]);
...
#pragma omp simd
        for (int j = 0; j < 8; j++) {
            ...
            importantParts[j] = dotProducts[j] * dotProducts[j] -
                                distancesSqr[j] + rad[i + j] * rad[i + j];
        }
...
#pragma omp simd
        for (int j = 0; j < 8; j++) {
            ...
            if (ds[j] < bestHit.distance) {
                ...
                bestHit.normal.y =
                    (py[i + j] - bestHit.point.y) * (1.0 / rad[i + j]);
                bestHit.normal.x =
                    (px[i + j] - bestHit.point.x) * (1.0 / rad[i + j]);
                bestHit.normal.z =
                    (pz[i + j] - bestHit.point.z) * (1.0 / rad[i + j]);
